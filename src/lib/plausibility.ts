...
export function checkFillup(input: PlausibilityInput): string[] {
  ...
  if (odometerKm != null && lastOdo != null && Math.round(odometerKm) <= lastOdo) {
    warnings.push(
      `Compteur en baisse — le dernier relevé était ${fmtKm(lastOdo)}. C'est un autre véhicule ?`,
    )
  }
  ...
  if (unitPrice != null && unitPrice > 0) {
    ...
    if (unitPrice < b.min || unitPrice > b.max) {
      warnings.push(`Prix inhabituel : ${b.fmt(unitPrice)}.`)
    }
  }
  ...
  if (
    ...
    if (avg != null && avg > 0 && Math.abs(per100 - avg) / avg > 0.4) {
      ...
      warnings.push(
        `${f(per100)} estimé — inhabituel (moyenne ${f(avg)}). ${energy === 'electric' ? 'Charge' : 'Plein'} partiel${energy === 'electric' ? 'le' : ''} ?`,
      )
    }
  }
